...
class Gui(Tk):
    ...
    class ScrollableCanvas:
        def __init__(self, gui, width=500, height=500, **options):
            ...
            self.xb = self.sb(command=self.canvas.xview, orient=HORIZONTAL,
                              sticky=E+W)
...
def split_options(options):
    ...
    gridopts = pop_options(options, gridnames)
...
class GuiCanvas(Canvas):
    ...
    which make canvas coordinates Cartesian (origin in the middle,
    ...
    def move(self, item, dx=0, dy=0):
        coords = self.trans([[dx, dy]])
        pos = coords[0]
        Canvas.move(self, item, pos[0], pos[1])
