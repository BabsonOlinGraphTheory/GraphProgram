...
No Holes: The generated labelings contain no holes, at the possible expense of having a larger span."
        tkinter.messagebox.showinfo("Help!  I need somebody...", helptxt)
...
\n\
  Bottom Bar\n\
    ...
    Easy setup or change grid for snapping."
        ...
        tkinter.messagebox.showinfo("Basic Help (P1/3)", basics)
        tkinter.messagebox.showinfo("More Advanced Help (P2/3)", more_advanced)
        tkinter.messagebox.showinfo("An overview of the Menus (P3/3)", about_the_menus)
...
Email iamtesch@gmail.com for feature requests, questions, or help."
        tkinter.messagebox.showinfo("About", abouttxt)
...
# #####################################################################################################
    ...
    def opensequence(self, event=None):
        #self.querysave()
        if not self.cancelled:
            file = tkinter.filedialog.askopenfile(parent=self,mode='rb',filetypes=[('Graph Sequence','*.graphs')],title='Choose a file')
            if file != None:
                self.backups = []
    ...
    def saveas(self, event=None, fileName=None):
        if not self.cancelled:
            if not fileName:
                fileName = tkinter.filedialog.asksaveasfilename(parent=self,filetypes=myFormats,title="Save the graph as...")
            if len(fileName ) > 0:
...
##                        tkMessageBox.showinfo("You need PIL!", "It looks like you don't have PIL.\nYou need the Python Imaging Library to save to an image!\nhttp://www.pythonware.com/products/pil/")
    ...
    def savesequence(self, event=None, fileName=None):
        if not self.cancelled:
            if not fileName:
                fileName = tkinter.filedialog.asksaveasfilename(parent=self,filetypes=[('Graph Sequence','*.graphs')],title="Save the graph sequence as...")
            if len(fileName ) > 0:
    ...
    def import_GPGs(self):
        file = tkinter.filedialog.askopenfile(parent=self,mode='r',title='Choose a file')
        if file !=None:
            self.clear()
    ...
    def enter_shortcode(self):
        res = tkinter.simpledialog.askstring("Enter Shortcode", "Enter Shortcode to load:")
        res=res.split(',')
        if len(res)==1:
            res=res[0].split()
        if len(res)==1:
    ...
    def enter_GPGpermutation(self):
        res = tkinter.simpledialog.askstring("Enter Permutation", "Enter permutation to load:")
    ...
    def process_regular_graph(self):
        infile = tkinter.filedialog.askopenfile(parent=self,mode='rb',title='Choose a file')
        outfilename = tkinter.filedialog.asksaveasfilename(parent=self,title="Save the output as...")
    ...
    def get_graph_representations(self):
        ...
        if len(self.graph.get_vertices())<10:
            tkinter.messagebox.showinfo("Graph Representations:",adjmessage+"\n"+spmmessage+"\n"+d2mmessage+"\n"+scmessage)
        else:
            tkinter.messagebox.showinfo("Graph Representations (P1/4):",adjmessage)
            tkinter.messagebox.showinfo("Graph Representations (P2/4):",spmmessage)
            tkinter.messagebox.showinfo("Graph Representations (P3/4):",d2mmessage)
            tkinter.messagebox.showinfo("Graph Representations (P4/4):",scmessage)
...
# #####################################################################################################
    ...
    def choose_color(self):
        ...
        if len([v for v in self.graph.get_vertices() if v.selected])>0:
            color=tkinter.colorchooser.askcolor(color=self.graph.get_vertices()[0].color,title="Choose a Vertex Color")[1]
        ...
        if len([edge for edge in self.graph.edges if edge.selected])>0:
            color=tkinter.colorchooser.askcolor(color=self.graph.edges[0].color,title="Choose an Edge Color")[1]
            for edge in [edge for edge in self.graph.edges if edge.selected]:
                edge.color=color
            self.redraw();
        if vert_selected:
            color=tkinter.colorchooser.askcolor(color=self.graph.get_vertices()[0].lcolor,title="Label Color")[1]
            if color!=None:
                for v in [v for v in self.graph.get_vertices() if v.selected]:
                    v.lcolor=color
        self.redraw()
    ...
    def pick_size(self, event=None):
        vert_selected=False
        if len([v for v in self.graph.get_vertices() if v.selected])>0:
            size=int(tkinter.simpledialog.askstring("Pick Size", "What's a good size for the selected vertices?"))
            if size!=None:
                size=int(size)
                for v in [v for v in self.graph.get_vertices() if v.selected]:
                    if size!=None:
                ...
                self.redraw();
        if len([edge for edge in self.graph.edges if edge.selected])>0:
            size=tkinter.simpledialog.askstring("Pick Size", "What's a good width for the selected edges?")
            if size!=None:
                size=int(size)
                for edge in [edge for edge in self.graph.edges if edge.selected]:
                    edge.size=size
                self.redraw();
        if vert_selected:
            size=tkinter.simpledialog.askstring("Pick Size", "What's a good width for the labels?")
            if size!=None:
                size=int(size)
                for v in [v for v in self.graph.get_vertices() if v.selected]:
                    v.lsize=size
...
# #####################################################################################################     
    ...
    def check_labeling(self,quiet = False):
        ...
        if res == True:
            ...
            if not quiet:
                ...
                tkinter.messagebox.showwarning("Bad labeling", "The selected vertices appears to be wrong.")
                self.redraw()
            return False
    ...
    def count_holes(self,quiet = False):
        ...
        if not quiet:
            self.control_up()
            tkinter.messagebox.showinfo("Number of Holes", "The number of holes in this labeling is " + str(numholes) + ".")
        return numholes
   ...
   #         print "copy"
    ...
    def clicked(self, event):
        ...
        if self.box:
            ...
            xcoords = (b0,b2)
            ycoords = (b1,b3)
            if self.clicked_on in list(self.handles.keys()):
                self.resize = ['m','m']
    ...
    def insert_copied(self, event):
        ...
        for v in list(vertdict.values()):
            v.toggle()
        for e in self.graph.edges:
            if e.vs[0] in list(vertdict.values()) and e.vs[1] in list(vertdict.values()):
                e.toggle()
        self.surrounded_vertices = list(vertdict.values())
...
# #####################################################################################################             
    ...
    def insert_subgraph(self):
            ...
            if subgraph.name == self.selected_subgraph:
                ...
                for i in range(len(subgraph.options)):
                    try:
                        ...
                        if option <= 0:
                            tkinter.messagebox.showwarning("Invalid Parameters", "All parameters should be positive!")
                            return
                        ...
                        try:
                            ...
                            tkinter.messagebox.showwarning("Invalid Parameters", "All parameters should be integers!")
                            return
        ...
        if self.selected_subgraph == "file":
            ...
            if file != None:
                ...
                self.redraw()
                return
            tkinter.messagebox.showwarning("Unable to Insert", "No File Chosen")
            return
        
        res = generate.generate(subgraph.name, options)
        if res[0] == "ERROR":
            tkinter.messagebox.showwarning("Invalid Parameters", res[1])
    ...
    def message(self, title, message, function, type="okcancel"):
        if type=="passgraphokcancel":
            res = tkinter.simpledialog.askinteger(title, message)
            if res != None:
