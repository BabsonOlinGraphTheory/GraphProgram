...
def midpoint(xxx_todo_changeme1, xxx_todo_changeme2):
...
class Vertex(Component):
    def __init__(self, xxx_todo_changeme,label="NULL",labelpos=(-12,-12)):
...
##            self.circle = canvas.create_oval(x-self.size, y-self.size, x+self.size, y+self.size, fill=self.color, activefill="blue", tags=(self.id,"vertex"), width = 0)
    ...
    def draw_to_PIL(self, canvas, width, height):
        ...
        if self.label!="NULL":
            ...
            font=ImageFont.truetype(os.path.join(fontdir,'ARIALBD.TTF'),self.lsize)
            try:
...
class Graph:
    ...
    def loadshortcode(self, shortcode, all=True):
        ...
        while i<len(shortcode):
            ...
            for j in range(k*n/2-numsame):
                currentcode.append(shortcode[i])
                i+=1
            degrees=[0]*n
            j=0
            for currentvertnum in range(n):#go through every vertex number from 0 to n-1
                for l in range(k-degrees[currentvertnum]):
